...
yeild db - gives that connection to whoever needs it
finally db.close - after the request is done, closes the connection

Depends(get_db) - tells fastAPI before running this function get the DB connection using get_db function
db - connection you have received
db.query - query the database using that connection
"""
def get_db():
    db = SessionLocal()
    try:
...
async def all(user: user_dependancy ,db: db_dependancy):
    if user is None:
